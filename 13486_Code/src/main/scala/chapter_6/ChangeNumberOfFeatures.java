...
public class ChangeNumberOfFeatures {
  ...
  private static void testAbilityOfModelToSumNumbers(MultiLayerNetwork net) {
    //we want to predict from three features - need to change number of classes in the layers
    final INDArray input = Nd4j.create(new double[]{0.111111, 0.3333333333333, 0.55555555}, new int[]{1, 3});
    INDArray out = net.output(input, false);
    System.out.println("nEpochs: " + nEpochs + " for input: " + input + " result = " + out);

    final INDArray input2 = Nd4j.create(new double[]{0.1, 0.8, 0.01}, new int[]{1, 3});
    INDArray out2 = net.output(input2, false);
    System.out.println("nEpochs: " + nEpochs + " for input: " + input2 + " result = " + out2);
  }
  ...
  private static MultiLayerNetwork configureMultiLayer() {
    //Create the network
    int numInput = 3; //should be equal to number of input features
    int numOutputs = 1; //we want only one input class
    int nHidden = 10;
  ...
  private static DataSetIterator generateTrainingSumData(int batchSize, Random rand) {
    ...
    for (int i = 0; i < nSamples; i++) {
      ...
      input3[i] = MIN_RANGE + (MAX_RANGE - MIN_RANGE) * rand.nextDouble();
