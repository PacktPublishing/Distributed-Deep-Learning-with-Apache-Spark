...
public class SumNumberOfIterations {
  ...
  private static List<Integer> nEpochsList = Arrays.asList(10, 100, 1000, 2000);

  public static void main(String[] args) {

    for(int nEpoch: nEpochsList) {
      DataSetIterator iterator = generateTrainingSumData(batchSize, rng);

      MultiLayerNetwork net = configureMultiLayer();

      trainModel(iterator, net, nEpoch);

      testAbilityOfModelToSumNumbers(net, nEpoch);
    }

  }

  private static void testAbilityOfModelToSumNumbers(MultiLayerNetwork net, int nEpochs) {
  ...
  private static void trainModel(DataSetIterator iterator, MultiLayerNetwork net, int nEpoch) {
